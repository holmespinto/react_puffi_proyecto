...
const MenuItemWithChildrenHoriz = ({ item, linkClassName, subMenuClassNames, activeMenuItems, toggleMenu }) => {
    const [open, setOpen] = useState(activeMenuItems.includes(item.key));
    ...
    const toggleMenuItem = (e) => {
        ...
        const status = !open;
        //const opul = url !== '/pages/' ? openul : !openul;
        const opul = !openul;
        setOpen(status);
        setOpenul(opul);
        if (toggleMenu) toggleMenu(item, status);
        ...
        <li className={classNames('side-nav-item', { 'menuitem-active': open })}>
            <Link
                ...
                {!item.badge ? (
                    ...
                    <span className={`badge bg-${item.badge.variant} float-end`}></span>
                )}
                <span> {item.label} </span>
            </Link>
                                ...
                                {child.children ? (
                                    <>
                                        ...
                                        <MenuItemWithChildren
                                            ...
                                            linkClassName={activeMenuItems.includes(child.key) ? 'active' : ''}
                                            activeMenuItems={activeMenuItems}
                                            subMenuClassNames="side-nav-one-level"
                                            toggleMenu={toggleMenu}
                                        />
                                    </>
                                ) : (
...
const AppMenu = ({ menuItems, location, isCondensed }: AppMenuProps) => {
    ...
    return (
        <>
            {isCondensed ? (
                <ul className="side-nav" id="main-side-menu" ref={menuRef}>
                ...
                (menuItems || []).map((item, idx) => {
                    return (
                        <ul className="side-nav" ref={menuRef} id="main-side-menu" key={idx}>
                            {item.isTitle ? (
